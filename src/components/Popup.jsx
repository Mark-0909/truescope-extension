...
const getColorClasses = (verdictLabel) => {
  switch (verdictLabel) {
    case "true":
      return {
        header: "bg-green-500",
...
export default function Popup({ overallVerdict, selectedText, scores }) {
  const [verdictLabel, setVerdictLabel] = useState(
    mapVerdictToLabel(overallVerdict),
  );
  const [colors, setColors] = useState(getColorClasses(verdictLabel));
  const [truthScore, setTruthScore] = useState(
    verdictToTruthScore(overallVerdict),
  );
    ...
    <div className="w-full h-screen bg-white text-gray-900 flex flex-col overflow-hidden">
      ...
      <div className="flex border-b-3 border-black/20">
        <div className="p-1 space-y-1 w-2/3">
          ...
          <BiasBar type="consistency" value={truthScore} />
        </div>
